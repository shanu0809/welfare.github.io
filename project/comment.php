...
class Comments {
  ...
  // PARAM $pid : post ID
    ...
    $sql = "SELECT * FROM `commentquery` WHERE `post_id`=? ORDER BY `reply_id`";
    $this->stmt = $this->pdo->prepare($sql);
  ...
  //       $rid : reply id
    ...
    $sql = "INSERT INTO `commentquery` ($fields) VALUES ($values);";
  ...
  //       $message : comment message
    ...
    $sql = "UPDATE `commentquery` SET `name`=?, `message`=? WHERE `comment_id`=?;";
  ...
  // PARAM $cid : comment ID
    ...
    $this->start();
    $pass = $this->exec("DELETE FROM `commentquery` WHERE `comment_id`=?;", [$cid]);
    if ($pass) {
      $pass = $this->exec("DELETE FROM `commentquery` WHERE `reply_id`=?;", [$cid]);
    }
    $this->end($pass);
    return $pass;
  }
